...
UTILITY_STATEMENTS = [
    UtilityStatement(
        ...
        summary_cards=[
            ...
            ("Amount Due", "$2,100.00", "Matched to a July ledger billing cycle"),
        ],
        charges=[
            ("Energy charge", dec("1362.15")),
            ("Delivery services", dec("365.80")),
            ("Regulatory and system charge", dec("130.37")),
            ("HST", dec("241.68")),
        ],
        footer_note="This statement is anchored to Maple Leaf Catering's July Toronto Hydro ledger entry so GreenLens can test document-to-ledger verification against a realistic monthly utility bill.",
    ),
    UtilityStatement(
        ...
        summary_cards=[
            ...
            ("Amount Due", "$842.17", "Matched to a January ledger billing cycle"),
        ],
        charges=[
            ("Commodity gas charge", dec("384.22")),
            ("Delivery and storage", dec("198.55")),
            ("Transportation to Enbridge", dec("72.14")),
            ("Federal carbon charge", dec("60.79")),
            ("HST", dec("126.47")),
        ],
        footer_note="This gas statement is tuned to Maple Leaf Catering's January Enbridge ledger entry so the assurance workflow can validate a clean utility match.",
    ),
    UtilityStatement(
        ...
        summary_cards=[
            ...
            ("Amount Due", "$830.00", "Matched to the August utility ledger entry"),
        ],
        charges=[
            ("Energy charge", dec("546.14")),
            ("Delivery services", dec("137.82")),
            ("Regulatory and system charge", dec("50.42")),
            ("HST", dec("95.62")),
        ],
        footer_note="This Toronto Hydro statement aligns with TechNorth Solutions' August electricity ledger entry so the fraud screen has a clean benchmark utility document.",
    ),
    UtilityStatement(
        ...
        summary_cards=[
            ...
            ("Amount Due", "$175.00", "Matched to the December gas ledger entry"),
        ],
        charges=[
            ("Commodity gas charge", dec("63.18")),
            ("Delivery and storage", dec("39.55")),
            ("Transportation to Enbridge", dec("18.76")),
            ("Federal carbon charge", dec("12.18")),
            ("HST", dec("41.33")),
        ],
        footer_note="This winter gas statement is aligned to TechNorth Solutions' December Enbridge ledger entry for clean document-assurance testing.",
    ),
]
...
VENDOR_INVOICES = [
    VendorInvoice(
        ...
        terms="Net 15",
        line_items=[
            ("Seasonal produce and greens for event prep", 1, dec("2205.40")),
            ("Protein order - poultry and braised meats", 1, dec("1864.35")),
            ("Prepared desserts and dairy restock", 1, dec("1548.25")),
            ("Dry goods, pantry staples, and kitchen consumables", 1, dec("1550.14")),
        ],
        tax_label="HST",
        tax_amount=dec("931.86"),
        notes="This polished supplier invoice is intentionally matched to a representative food-supply ledger amount so GreenLens can demonstrate successful document verification alongside fraud screening.",
    ),
    VendorInvoice(
        ...
        terms="Net 15",
        line_items=[
            ("EC2 compute workload for production services", 1, dec("1896.40")),
            ("RDS database instances and backups", 1, dec("1024.25")),
            ("S3 storage, requests, and data transfer", 1, dec("637.40")),
            ("Developer support plan and monitoring", 1, dec("380.00")),
        ],
        tax_label="HST",
        tax_amount=dec("511.95"),
        notes="This AWS invoice is matched to the October cloud-hosting ledger entry so the assurance workflow can show a high-confidence vendor and amount reconciliation.",
    ),
    VendorInvoice(
        relative_path="demo_data/support_docs/maple_leaf_catering/maple_leaf_sysco_invoice_sep_2024_duplicate.pdf",
        brand="Sysco Toronto",
        subtitle="Foodservice Distribution",
        accent=(0.0, 0.38, 0.62),
        title="Supplier Invoice",
        ...
        tax_label="HST",
        tax_amount=dec("931.86"),
        notes="Intentional duplicate copy for fraud-signal testing. Upload this only when you want GreenLens to surface a duplicate-invoice finding.",
    ),
    VendorInvoice(
        relative_path="demo_data/support_docs/technorth_solutions/technorth_aws_invoice_oct_2024_duplicate.pdf",
        brand="Amazon Web Services",
        subtitle="Cloud Services Billing",
        accent=(0.94, 0.55, 0.1),
        title="Services Invoice",
        ...
        tax_label="HST",
        tax_amount=dec("511.95"),
        notes="Intentional duplicate copy for fraud-signal testing. Upload this alongside the main AWS invoice to trigger a duplicate-reference alert.",
    ),
]
...
FUEL_RECEIPTS = [
    FuelReceipt(
        ...
        odometer="184,233 km",
        line_items=[
            ("Ultra low sulphur diesel - fleet pump batch", dec("438.20")),
            ("DEF top-up, car wash, and service fees", dec("56.80")),
        ],
        total=dec("495.00"),
        notes=[
            "Representative fleet fuel event aligned to Maple Leaf Catering's August Shell ledger entry.",
            "Useful for demonstrating a clean vendor/date/amount match in the document assurance flow.",
        ],
    ),
    FuelReceipt(
        ...
        line_items=[
            ...
            ("Handwritten surcharge adjustment", dec("54.90")),
        ],
        total=dec("73.25"),
        notes=[
            "Intentional fraud-signal sample: the line items do not reconcile to the stated total.",
            "The vendor also does not map cleanly to TechNorth's uploaded travel ledger, so it should trigger follow-up.",
        ],
    ),
    FuelReceipt(
        relative_path="demo_data/support_docs/maple_leaf_catering/maple_leaf_shell_fleet_receipt_aug_2024_duplicate.pdf",
        brand="Shell Canada",
        subtitle="Fleet Card Fuel Receipt",
        accent=(0.79, 0.1, 0.12),
        company_name="Maple Leaf Catering Co.",
        ...
        line_items=[
            ("Ultra low sulphur diesel - fleet pump batch", dec("438.20")),
            ("Handwritten DEF adjustment and wash add-on", dec("94.35")),
        ],
        total=dec("495.00"),
        notes=[
            "Intentional duplicate fraud-signal sample using the same merchant reference as the clean Shell receipt.",
            "The line items also fail to reconcile to the stated total, so GreenLens should surface both duplicate and math mismatch alerts.",
        ],
    ),
]
