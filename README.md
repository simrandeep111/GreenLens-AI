...
The user lands on the intake page and enters company details.
The user uploads a CSV and optionally uploads supporting PDFs.
The frontend calls the upload endpoint and then starts the analysis job.
The backend runs the pipeline in a background thread and updates progress.
The frontend polls the status endpoint and shows each processing stage.
Once complete, the dashboard and full report become available.
The user can review emissions, compliance, funding matches, recommendations, and supporting-document findings.
The user can ask follow-up questions in the report copilot, which answers from the saved report and retrieved context.
...
Keyword rules
Default Scope 3 fallback with subcategory detection
...
The backend loads the saved report.
It converts report sections, emissions breakdowns, flags, documents, compliance items, grants, and recommendations into retrieval chunks.
It optionally augments with compliance and grant retrieval.
It answers the question using those chunks as context.
If the LLM call fails, it falls back to a deterministic grounded answer.
